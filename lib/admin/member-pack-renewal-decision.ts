...
export function getRemainingSessionsForPack(state: MemberPackState): number {
  ...
  // Pas de solde enregistré ≠ pack plein inutilisé (évite « 5 restantes » fantômes
  // quand toutes les inscriptions sont Terminées / Remplacées).
  return 0;
}
 ...
 */
export function decidePackRenewal(
  state: MemberPackState,
  today: Date = startOfLocalToday(),
): PackRenewalDecision {
  if (!state.packId) {
...
export function packRenewalMessageFr(decision: PackRenewalDecision, queuedPackName?: string): string {
  if (decision.mode === "immediate") {
    ...
    // Terminé (0 séance) ≠ Expiré (date dépassée avec séances restantes).
    if (decision.remainingSessions <= 0) {
